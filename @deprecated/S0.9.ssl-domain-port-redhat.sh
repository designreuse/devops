...
BASE_INSTALL=/home/ec2-user/devops
TMP_INSTALL=/tmp/devops-install
NGINX_CONF=$BASE_INSTALL/_redhat/etc/nginx
...
create_ssl() {
	...
	if sudo test -f /etc/letsencrypt/live/$local_domain/fullchain.pem; then
		...
		# sudo letsencrypt certonly --webroot -w /opt/letsencrypt -d $local_domain --email digital@vboss.tech --agree-tos
...
#      if [[ $local_port =~ '^[0-9]+$' ]]; then
        if [[ "$local_domain" == *"alfresco"* ]] || [[ "$local_domain" == *"camunda"* ]]; then
			...
			ssl_found=0
			if [ -f "/etc/nginx/conf.d/$local_domain.conf" ]; then
				ssl_found=$(grep -o "443" /etc/nginx/conf.d/$local_domain.conf | wc -l)
			fi

			if [ "$ssl_found" = 0 ]; then
				sudo rsync -avz $NGINX_CONF/conf.d/domain.conf.ssl /etc/nginx/conf.d/$local_domain.conf
				# sudo ln -s /etc/nginx/conf.d/$local_domain.conf /etc/nginx/sites-enabled/

				#sudo sed -i "s/@@WEB_ROOT@@/${WEB_ROOT//\//\\/}/g" /etc/nginx/conf.d/$local_domain.conf
				sudo sed -i "s/@@DNS_DOMAIN@@/$local_domain/g" /etc/nginx/conf.d/$local_domain.conf

				if [[ "$local_domain" == *"alfresco"* ]]; then
					# Insert cache config
					sudo sed -i '1 i\proxy_cache_path \/var\/cache\/nginx\/alfresco levels=1 keys_zone=alfrescocache:256m max_size=512m inactive=1440m;\n' /etc/nginx/conf.d/$local_domain.conf
					sudo sed -i "0,/server/s/server/upstream alfresco {	\n\tserver localhost\:$local_port;	\n}	\n\n upstream share {    \n\tserver localhost:$local_port;	\n}\n\n&/" /etc/nginx/conf.d/$local_domain.conf
					sudo sed -i "s/##REWRITE##/rewrite \^\/\$	\/share;/g" /etc/nginx/conf.d/$local_domain.conf
					sudo cp $NGINX_CONF/conf.d/alfresco.snippet temp/
					sudo sed -e '/##ALFRESCO##/ {' -e 'r temp/alfresco.snippet' -e 'd' -e '}' -i /etc/nginx/conf.d/$local_domain.conf
					...
					sudo sed -i "s/##REWRITE##/rewrite \^\/\$	\/camunda;/g" /etc/nginx/conf.d/$local_domain.conf
					sudo cp $NGINX_CONF/conf.d/camunda.snippet temp/
					sudo sed -e '/##CAMUNDA##/ {' -e 'r temp/camunda.snippet' -e 'd' -e '}' -i /etc/nginx/conf.d/$local_domain.conf
				fi
			fi
			...
			sudo sed -i '1 i\proxy_cache_path \/var\/cache\/nginx\/alfresco levels=1 keys_zone=alfrescocache:256m max_size=512m inactive=1440m;\n' /etc/nginx/conf.d/$local_domain.conf
			sudo sed -i "s/##REWRITE##/rewrite \^\/\$	\/cashflow;/g" /etc/nginx/conf.d/$local_domain.conf
			...
			sudo cp $NGINX_CONF/conf.d/cashflow.snippet	temp/
			sudo sed -e '/##CASHFLOW##/ {' -e 'r temp/cashflow.snippet' -e 'd' -e '}' -i /etc/nginx/conf.d/$local_domain.conf
			...
			if sudo test -f $FILE; then
				...
				sudo cp $NGINX_CONF/conf.d/magento.snippet /tmp/
				...
				sudo cp /tmp/magento.snippet /etc/nginx/conf.d/$local_domain.conf
				# sudo ln -s /etc/nginx/conf.d/$local_domain.conf /etc/nginx/sites-enabled/
			else
			...
			sudo sed -i "s/##WEB_ROOT##/root $DEVOPS_HOME_PATH\/eformsrenderer\/dist;/g" /etc/nginx/conf.d/$local_domain.conf
		else
			sudo rsync -avz $NGINX_CONF/conf.d/domain.conf.ssl /etc/nginx/conf.d/$local_domain.conf
			# sudo ln -s /etc/nginx/conf.d/$local_domain.conf /etc/nginx/sites-enabled/

			sudo sed -i "s/@@DNS_DOMAIN@@/$local_domain/g" /etc/nginx/conf.d/$local_domain.conf
			sudo cp $NGINX_CONF/conf.d/common.snippet temp/
			sudo sed -e '/##COMMON##/ {' -e 'r temp/common.snippet' -e 'd' -e '}' -i /etc/nginx/conf.d/$local_domain.conf

			sudo sed -i "s/@@PORT@@/$local_port/g" /etc/nginx/conf.d/$local_domain.conf
		fi
	   fi
