...
if [ ! -d "$DEVOPS_HOME/eforms/bpmn" ]; then
    ...
    sudo sed -i "s/\(^eform.cli.deployBpmn=\).*/\1true/" 	$TMP_INSTALL/workplacebpm/src/workforce-dmn-xlsx/xlsx-dmn-cli/src/main/resources/application.properties
fi
...
if [ ! -f "$DEVOPS_HOME/eforms/dmn/input/RACI-Decision-Making-Criteria.xlsx" ]; then
    ...
    sudo sed -i "s/\(^eform.cli.deployDmn=\).*/\1true/" 	$TMP_INSTALL/workplacebpm/src/workforce-dmn-xlsx/xlsx-dmn-cli/src/main/resources/application.properties
fi
...
sudo sed -i "s/\(^eform.cli.bpmn.deployFolderPath=\).*/\1$BPMN_PATH_ESC/" 	$TMP_INSTALL/workplacebpm/src/workforce-dmn-xlsx/xlsx-dmn-cli/src/main/resources/application.properties
...
sudo sed -i "s/\(^eform.cli.dmn.camunda.url.deployment=\).*/\1$camunda_protocol:\/\/$camunda_hostname\/engine-rest\/engine\/$TENANT_ID\/deployment\/create/" 	$TMP_INSTALL/workplacebpm/src/workforce-dmn-xlsx/xlsx-dmn-cli/src/main/resources/application.properties
...
sudo java -jar $TMP_INSTALL/workplacebpm/src/workforce-dmn-xlsx/xlsx-dmn-cli/target/dmn-xlsx-cli-0.2.1-SNAPSHOT.jar
