...
if [ ! -f "/usr/bin/gs" ]; then
  ...
  wget $GHOSTSCRIPTURL -P $TMP_INSTALL/
  echo "Extracting..."
  sudo tar -xvzf $TMP_INSTALL/ghostscript-9.23-linux-x86_64.tgz -C $TMP_INSTALL
  sudo mv $TMP_INSTALL/ghostscript-9.23-linux-x86_64/gs-923-linux-x86_64 /usr/bin/gs
...
if [ -d "$CATALINA_HOME" ]; then
	...
	alfresco_line=$(grep "ecashflow" $BASE_INSTALL/domain.txt)
	IFS='|' read -ra arr <<<"$alfresco_line"
	alfresco_port="$(echo -e "${arr[3]}" | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')"
	SHARE_HOSTNAME="$(echo -e "${arr[2]}" | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//')"
	echo "share hostname : $SHARE_HOSTNAME"
